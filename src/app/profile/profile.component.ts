...
export class ProfileComponent implements OnInit {
  [key: string]: any;
  skills: {SkillName: string, yearsOfExperience: number, _id: number}[] = [];
...
careerLevels = [  "Student",  "Entry Level",  "Experienced",  "Manager",  "Senior Management"];
selectedJobType: string | undefined;
jobTypeOptions = ["Full Time", "Part Time", "Freelance", "internship", "Remotely", "Volunteering"];
selectedEducationalLevel :string | undefined;
educationalLevels = [  "Bachelors Degree",  "Master Degree",  "Doctorate Degree",  "High school",  "Vocational",  "Diploma"];
  constructor(private user:UserService) { }
  emailFormControl = new FormControl('', [Validators.required, Validators.email]);
  birthdateFormControl = new FormControl();
  nameFormControl = new FormControl('', [Validators.required]);
  numberFormControl = new FormControl('', [Validators.required,Validators.pattern(
  ...
  yearsOfExperienceSkillFormControl = new FormControl('', [Validators.required]);
  ...
  ngOnInit(): void {
    this.user.getUserInfo().subscribe(res=>{
      ...
      this.setValue('GraduationYearFormControl', this.profileData.GraduationYear);
      this.setValue('gradeFormControl', this.profileData.Grade);
      this.setValue('industryFormControl', this.profileData.industry);
      this.setValue('yearsOfExperienceFormControl', this.profileData.yearsOfExperience);
      this.setValue('currentCareerlevelFormControl', this.profileData.currentCareerlevel);
      ...
      for (let i = 0; i < this.profileData.jobTitles.length; i++) {
        this.fruits.push({name : this.profileData.jobTitles[i]})
      }
      this.image = this.profileData.image
    });
  }
  addSkill() {
    if (this.skillNameFormControl.valid && this.yearsOfExperienceSkillFormControl.valid) {
      const newSkill = { SkillName: this.skillNameFormControl.value!, yearsOfExperience: +this.yearsOfExperienceSkillFormControl.value!, _id: 0 };
